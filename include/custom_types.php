...
add_action('init', 'register_post_cours');
function register_post_cours(){
    register_post_type('cours', array(
        ...
        'hierarchical'        => false,
        'supports'            => array('title', 'editor',  'thumbnail','excerpt'), // 'title','editor','author','thumbnail','excerpt','trackbacks','custom-fields','comments','revisions','page-attributes','post-formats'
        'taxonomies'          => array('course_cat', 'language'),
        'has_archive'         => true,
        'rewrite'             => true,
        'query_var'           => true,
    ) );
...
// хук для регистрации
add_action('init', 'create_taxonomy_cat_course');
function create_taxonomy_cat_course(){
    // заголовки
    $labels = array(
        'name'              => 'Категори курсов',
        'singular_name'     => 'Категори курсов',
        'search_items'      => 'Искать категорию',
        'all_items'         => 'Все категории',
        'parent_item'       => 'Родительская категория',
        'parent_item_colon' => 'Родительская категория:',
    ...
    register_taxonomy('course_cat', array('cours'), $args );
}
